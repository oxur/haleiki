...
Defined per-deployment. For the Rust knowledge base:

```
Categories:
  memory-model, type-system, concurrency, error-handling,
  fundamentals, data-structures, patterns-idioms, tooling,
  ecosystem, unsafe-rust, macros, testing
...
│   └── taxonomy.yaml
├── _analysis/                      # Persisted extraction metadata
...
| Role | Typeface | Usage |
|------|----------|-------|
| Display / Headings | DM Sans | Article titles, section heads, nav, UI labels |
| Body / Long-form | Source Serif 4 | Article prose, definitions, examples |
...
3. Image/diagram strategy (SVG inline? build-time generated?)
